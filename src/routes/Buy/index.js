...
async function createOrder(selected) {
  console.log("createOrder", selected);
  const cookies = getCookie("connect.sid");
  let response = await axios.post(window.APIROOT + "api/user/createOrder", {
    selected: selected,},{
...
function PayButton(props) {
  ...
  return (
    <Button
      disabled={props.disabled || !props.cost}
...
export default function BuyPage() {
  ...
  useEffect(() => {
    ...
    for (const [day, val] of Object.entries(selected)) {
      for (const [meal, mealV] of Object.entries(val)) {
        if (mealV) cost += menu[dayToNum[day.toLowerCase()]][meal]["cost"];
      }
    }
    setCost(cost);
  }, [menu, selected]);
    ...
    const fetchData = async () => {
      try {
        ...
        for (let i = 0; i < response.data.length; i++) {
          response.data[i].breakfast = {
            text: response.data[i].breakfast,
            cost: data.breakfast,
          };
          response.data[i].lunch = {
            text: response.data[i].lunch,
            cost: data.lunch,
          };
          response.data[i].dinner = {
            text: response.data[i].dinner,
            cost: data.dinner,
          };
        }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const cookies = getCookie("connect.sid");
        const response = await axios.get(window.APIROOT + "api/user/boughtNextWeek", {
          withCredentials: true, // Important: Set this to true
          headers: {
            Cookie: cookies,
          },});
    ...
    <>
      {bought ? (
        <div className={classes.bought}>
          ...
          >
            You can buy coupons for a week, the week before. You have already
            bought coupons for the next week.
          </Card>
        </div>
      ) : (
